  ...
  function buildHealthPanelHtml(data) {
    ...
    const feishuInfo = !feishu.tokenPresent
      ...
      : feishu.tokenExpiredSoon
        ...
        : {
            cls: "hv-ok",
            txt: `✅ Token 有效 <em>· ${Math.floor(feishu.expiresInMinutes / 60)}h ${feishu.expiresInMinutes % 60}m</em>`,
          };
...
<div class="health-card">
  ...
  window.setupCheckServer = function () {
    setupCheckServer();
  };
  ...
  function renderSetupWizard() {
    ...
    SETUP_STEPS.forEach(function (step, i) {
      var cls =
        i < setupState.step
          ? "sw-stepper__step sw-stepper__step--completed"
          : i === setupState.step
            ? "sw-stepper__step sw-stepper__step--active"
            : "sw-stepper__step";
      html += '<div class="' + cls + '">';
      ...
      if (i < SETUP_STEPS.length - 1) {
        html +=
          '<div class="sw-stepper__connector' +
          (i < setupState.step ? " sw-stepper__connector--done" : "") +
          '"></div>';
      }
    });
    html += "</div>";
  ...
  function buildSetupStepHtml(step) {
    switch (step) {
      ...
      case 2:
        return buildChannelStep();
      case 3:
        return buildLLMStep();
      case 4:
        return buildOptionalStep();
      case 5:
  ...
  function buildPinStep() {
    ...
    function statusIcon(v) {
      ...
      if (v.state === "checking") {
        return '<span class="sw-spinning">⟳</span>';
      }
      if (v.state === "valid") {
        return '<span class="sw-check-pop">✅</span>';
      }
      if (v.state === "error") {
        return "❌";
      }
    ...
    function inputCls(v) {
      if (v.state === "valid") {
        return "sw-token-input sw-token-input--valid";
      }
      if (v.state === "error") {
        return "sw-token-input sw-token-input--error";
      }
    ...
    if (setupState.serverStatus === "unreachable") {
      serverBanner =
        '<div class="sw-alert sw-alert--error" style="margin-bottom:1rem">' +
        "❌ 無法連接服務器（" +
        (cfg.serverUrl || "未設定") +
        "）<br>" +
      ...
      serverBanner =
        '<div class="sw-alert sw-alert--warn" style="margin-bottom:1rem">' +
        "⚠️ 瀏覽器安全限制：HTTPS 頁面無法直接連接 HTTP 服務器。<br>" +
        "請使用本地地址訪問設定向導：<br>" +
        '<a href="' +
        localUrl +
        '" style="color:var(--lp-primary);word-break:break-all">' +
        localUrl +
        "</a>" +
        "</div>";
    }
    ...
    return (
      ...
      (setupState.data.pin || "") +
      '" autocomplete="off" />' +
      '<span class="sw-token-input__status">' +
      statusIcon(pinValidation) +
      "</span>" +
      '<div class="sw-token-input__detail">' +
      (pinValidation.detail || pinValidation.error || "") +
      "</div>" +
      "</div>" +
      ...
      (function () {
        var pinMissing =
          setupState.validation["setup-pin"] && setupState.validation["setup-pin"].pinMissing;
        return pinMissing
      ...
      (pinValidation.state === "valid" ? "" : "disabled") +
      ">確認 →</button>" +
      "</div>" +
      "</div>"
    );
  ...
  function buildChannelStep() {
    ...
    return (
      ...
      "</details>" +
      '<div class="' +
      inputCls(tokenValidation) +
      '">' +
      '<label class="sw-token-input__label">Bot Token</label>' +
      '<input type="text" id="sw-tg-token" placeholder="123456:ABCdef..." value="' +
      tokenVal +
      '" autocomplete="off" />' +
      '<span class="sw-token-input__status">' +
      statusIcon(tokenValidation) +
      "</span>" +
      '<div class="sw-token-input__detail">' +
      detailTxt(tokenValidation) +
      "</div>" +
      "</div>" +
      '<details class="sw-guide">' +
      ...
      "</details>" +
      '<div class="' +
      inputCls(userIdValidation) +
      '">' +
      '<label class="sw-token-input__label">你的 Telegram User ID</label>' +
      '<input type="text" id="sw-tg-userid" placeholder="1234567890" value="' +
      userIdVal +
      '" autocomplete="off" inputmode="numeric" />' +
      '<span class="sw-token-input__status">' +
      statusIcon(userIdValidation) +
      "</span>" +
      '<div class="sw-token-input__detail">' +
      detailTxt(userIdValidation) +
      "</div>" +
      "</div>" +
      '<div class="sw-nav-bar">' +
  ...
  function buildLLMStep() {
    ...
    var providerCards = providers
      .map(function (p) {
        var selected = p.id === selectedProvider ? " sw-provider-card--selected" : "";
        return (
          '<div class="sw-provider-card' +
          selected +
          '" data-provider="' +
          p.id +
          '">' +
          '<span class="sw-provider-card__icon">' +
          p.icon +
          "</span>" +
          '<div class="sw-provider-card__name">' +
          p.name +
          "</div>" +
          '<div class="sw-provider-card__tag">' +
          p.tag +
          "</div>" +
          "</div>"
        );
    ...
    return (
      ...
      '<div class="sw-provider-grid" id="sw-provider-grid">' +
      providerCards +
      "</div>" +
      '<div class="' +
      inputCls(keyValidation) +
      '">' +
      '<label class="sw-token-input__label">API Key</label>' +
      '<input type="text" id="sw-llm-key" placeholder="sk-..." value="' +
      keyVal +
      '" autocomplete="off" />' +
      '<span class="sw-token-input__status">' +
      statusIcon(keyValidation) +
      "</span>" +
      '<div class="sw-token-input__detail">' +
      (keyValidation.detail || keyValidation.error || "") +
      "</div>" +
      "</div>" +
      (guideText
        ? '<details class="sw-guide"><summary>📖 如何取得 API Key？</summary>' +
          '<div class="sw-guide__body">' +
          '<div class="sw-guide__step"><div class="sw-guide__num">1</div><div>打開 <code>' +
          guideText +
          "</code></div></div>" +
      ...
      '<button class="sw-btn sw-btn--skip" id="sw-skip">跳過</button>' +
      '<button class="sw-btn sw-btn--primary" id="sw-next" ' +
      (keyValidation.state === "valid" ? "" : "disabled") +
      ">下一步 →</button>" +
      "</div>" +
      "</div>"
    );
  ...
  function buildOptionalStep() {
    return (
      ...
      '<input type="text" id="sw-feishu-appid" placeholder="cli_..." value="' +
      (setupState.data.feishu.appId || "") +
      '" />' +
      "</div>" +
      '<div class="sw-token-input">' +
      '<label class="sw-token-input__label">App Secret</label>' +
      '<input type="password" id="sw-feishu-secret" placeholder="••••••••" value="' +
      (setupState.data.feishu.appSecret || "") +
      '" />' +
      "</div>" +
      ...
      '<input type="password" id="sw-gw-token" placeholder="自定義密鑰" value="' +
      (setupState.data.gateway.authToken || "") +
      '" />' +
      "</div>" +
      "</div>" +
      "</details>" +
  ...
  function buildReviewStep() {
    var services = [
      {
        ...
        detail: setupState.data.telegram.botToken
          ? "Bot token 已設定" +
            (setupState.validation["telegram-token"]?.detail
              ? " · " + setupState.validation["telegram-token"].detail
              : "")
          : "未設定（跳過）",
      },
    ...
    var cardsHtml = services
      .map(function (s) {
        var statusEmoji = { ok: "🟢", warn: "🟡", error: "🔴", skip: "⚪" };
        return (
          '<div class="sw-service-card sw-service-card--' +
          s.state +
          '">' +
          '<div class="sw-service-card__icon">' +
          s.icon +
          "</div>" +
          '<div class="sw-service-card__body">' +
          '<div class="sw-service-card__name">' +
          s.name +
          "</div>" +
          '<div class="sw-service-card__detail">' +
          s.detail +
          "</div>" +
          "</div>" +
          '<div class="sw-service-card__status">' +
          (statusEmoji[s.state] || "⚪") +
          "</div>" +
          "</div>"
        );
    ...
    if (feishuAppId && feishuSecret) {
      function validateFeishu() {
        ...
        var setupHeaders = {
          ...
          "X-Setup-Pin": setupState.data.pin || "",
        };
        fetch(baseUrl + "/api/setup/validate", {
          ...
          .then(function (data) {
            ...
            if (statusEl) {
              if (data.ok) {
                var appName =
                  data.details && data.details.appName ? data.details.appName : "驗證成功";
                statusEl.innerHTML =
                  '<span style="color:var(--lp-success)">✅ ' + appName + "</span>";
              } else {
                statusEl.innerHTML =
                  '<span style="color:var(--lp-danger)">❌ ' +
                  (data.error || "驗證失敗") +
                  "</span>";
              }
            }
          ...
          .catch(function () {
            var statusEl = document.getElementById("sw-feishu-status");
            if (statusEl) {
              statusEl.innerHTML = '<span style="color:var(--lp-muted)">❓ 連接失敗</span>';
            }
          });
      }
      var feishuTimer;
  ...
  function setupTokenInputValidation(inputId, validationType) {
    ...
    input.addEventListener("paste", function () {
      ...
      requestAnimationFrame(function () {
        ...
        setTimeout(function () {
          input.parentElement.classList.remove("sw-paste-flash");
        }, 600);
      });
    ...
    fetch(baseUrl + "/api/setup/validate", {
      ...
      .then(function (data) {
        if (data.ok) {
          var detail = "";
          if (data.details) {
            if (data.details.botUsername) {
              detail = "Bot: @" + data.details.botUsername;
            } else if (data.details.userId) {
              detail = "ID: " + data.details.userId;
            } else if (data.details.models) {
              detail = "Models: " + data.details.models.slice(0, 2).join(", ");
            }
          }
          setupState.validation[type] = { state: "valid", detail: detail };
        } else {
          setupState.validation[type] = {
            state: "error",
  ...
  function collectCurrentStepData() {
    ...
    if (step === 0) {
      ...
      if (pinInput) {
        setupState.data.pin = pinInput.value.trim().toUpperCase();
      }
    } else if (step === 2) {
      var tgToken = document.getElementById("sw-tg-token");
      var tgUser = document.getElementById("sw-tg-userid");
      if (tgToken) {
        setupState.data.telegram.botToken = tgToken.value.trim();
      }
      if (tgUser) {
        setupState.data.telegram.userId = tgUser.value.trim();
      }
    } else if (step === 3) {
      var llmKey = document.getElementById("sw-llm-key");
      if (llmKey) {
        setupState.data.llm.apiKey = llmKey.value.trim();
      }
    } else if (step === 4) {
      ...
      if (fsAppId) {
        setupState.data.feishu.appId = fsAppId.value.trim();
      }
      if (fsSecret) {
        setupState.data.feishu.appSecret = fsSecret.value.trim();
      }
      if (gwToken) {
        setupState.data.gateway.authToken = gwToken.value.trim();
      }
    }
  }
  ...
  async function applySetupConfig() {
    ...
    try {
      ...
      if (data.ok) {
        ...
        if (errEl) {
          errEl.textContent = data.error || "寫入失敗";
          errEl.style.display = "block";
        }
        if (launchBtn) {
          launchBtn.disabled = false;
          launchBtn.textContent = "🚀 寫入配置";
        }
      }
    } catch (err) {
      if (errEl) {
  ...
  function renderServiceHealth() {
    ...
    if (cfg.mode !== "live") {
      container.innerHTML =
        '<div style="font-size:0.8rem;color:var(--lp-muted);padding:0.5rem 0;">連接到 Server 後可查看服務健康狀態</div>';
      return;
    }
    container.innerHTML =
      '<div style="font-size:0.8rem;color:var(--lp-muted);padding:0.5rem 0;">載入中...</div>';
    fetch((cfg.serverUrl || "") + "/api/setup/status")
      ...
      .then(function (data) {
        if (!data.ok) {
          container.innerHTML = '<div style="font-size:0.8rem;color:#ef4444;">無法取得狀態</div>';
          return;
        }
        var h = data.health || {};
      ...
      .catch(function () {
        container.innerHTML =
          '<div style="font-size:0.8rem;color:var(--lp-muted);">無法連接到 Server</div>';
      });
  }
