...
- Search framing source map (do not hardcode static keywords):
  - `company_focus`: target brand for this run
...
- Use concrete terms only when supported by materials, e.g., AI-agent workloads, wearable companion workflows, multimodal memory, creator stack interoperability.
...
- If context includes artifacts from `prompt_web_search_immersive.sh`, use these entries first:
  ...
  - The runner already encodes material context from this company (`https://lazying.art`, GitHub repos, local resources); do not inject brand-name-only search terms.
