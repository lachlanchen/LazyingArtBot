...
on buildExactSet(senderExactGlobal, acctExactList)
...
on run argv
  ...
  my logLine("account_filter=" & accountFilter & " lookbackDays=" & lookbackDays)
  ...
  tell application "Mail"
    repeat with acc in every account
      ...
      if accountFilter is not "" and accKey is not accountFilter then
        ...
        set exactSet to my buildExactSet(senderExactGlobal, acctExactList)
        ...
        repeat with mb in inboxes
          ...
          try
            set msgList to (messages of mb whose date received ≥ cutoffDate)
          on error
            set msgList to {}
          end try

          set msgCount to count of msgList
          my logLine("scan mailbox=" & mbName & " messages=" & msgCount)
          ...
          repeat with i from msgCount to 1 by -1
            ...
            if my shouldDelete(exactSet, senderContains, subjectContains, senderLine, subjectLine) then
              ...
              try
                ...
                if totalDeleted mod 25 is 0 then my logLine("deleted deleted=" & totalDeleted & " scanned=" & totalScanned & " last_sender=" & senderLine)
              on error errMsg number errNum
                my logLine("delete_error code=" & errNum & " msg=" & errMsg & " sender=" & senderLine & " subject=" & subjectLine)
              end try
            end if

            if totalScanned mod progressEvery is 0 then
              my logLine("progress scanned=" & totalScanned & " matched=" & totalMatched & " deleted=" & totalDeleted)
            end if
          end repeat
        end repeat
      end if
  ...
  my logLine("done scanned=" & totalScanned & " matched=" & totalMatched & " deleted=" & totalDeleted)
  return "done scanned=" & totalScanned & " matched=" & totalMatched & " deleted=" & totalDeleted
end run
