...
| `scripts/capture/notebooklm-tool-generate.ts` | `pnpm moltbot:capture:notebooklm-tool-generate` |
...
- Tool contract is stable:
  ...
  - Use `pnpm moltbot:capture:notebooklm-tool-generate` to call Google Generative Language (NotebookLM-compatible) endpoints directly from the worker; set `NOTEBOOKLM_API_KEY` (or `NOTEBOOKLM_ACCESS_TOKEN`), optionally override `NOTEBOOKLM_API_ENDPOINT`, `NOTEBOOKLM_MODEL`, and `NOTEBOOKLM_TEMPERATURE` according to your project.
