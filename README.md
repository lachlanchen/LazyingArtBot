...
**问题**：目前 Kairo 数据目录写死在 `/opt/LazyingArtBot/`，依赖 root 权限。**没有技术背景的人无法部署。私有 AI 秘书变成了技术人员的专属特权。**
...
- 去除 root 依赖，`$KAIRO_HOME` 可配置（**当前最高优先级，阻塞公开发布**）
- `install.sh` + AI 引导式交互 setup
- `openclaw.example.json` 完整模板 + GitHub Release v1.0
