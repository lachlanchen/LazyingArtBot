...
`install.sh` 会自动完成：Node.js 版本检查、pnpm 安装、依赖安装、UI 构建、主程序构建，然后启动交互式配置向导。

> **手动安装**（可选）：`pnpm install && pnpm ui:build && pnpm build`，然后运行 `scripts/setup.sh`

> **配置文件模板**：repo 根目录的 `openclaw.example.json` 是最小配置示例，复制到 `~/.openclaw/openclaw.json` 后填入你的参数即可。
> 支持 `KAIRO_HOME` 环境变量指定 Kairo 数据目录（默认 `~/.openclaw`）：
>
> ```bash
> export KAIRO_HOME=/your/custom/path
> ```
...
区别在哪里？你使用工具，意味着不用的时候它是关机状态。你依赖基础设施，意味着不论你在不在，它都在为你运作。

**电话是工具。电网是基础设施。**

现有的所有 AI——ChatGPT、Claude、Notion AI、各种 Agent 框架——本质上都是电话：你拨出去，有人接；你挂掉，连接断开。再聪明的电话，也只有你主动拨打才有价值。

Kairo 要做的是电网：一套**持续运行在你服务器上的个人 AI 协调层**。它在你睡觉时处理邮件摘要，在你开会时追踪到期任务，在你分心时提醒你三天前遗忘的那件事。

---

### 已验证的核心假设

Kairo v0.9 正在 Ken 的服务器上生产运行，已验证三件关键的事：

1. **主动推送可行** — 每天 07:10 晨报准时推到 Telegram，Ken 不需要做任何事
2. **完整闭环可行** — 说一件事 → 自动建卡 → 自动排程 → 到时间主动提醒 → LLM 自动更新状态，全程无需人工干预
3. **持久上下文可行** — 每次对话 Kairo 已感知今天的日历、未完成任务、相关人脉，不需要你重复介绍自己

这三件事合在一起意味着：**一个不需要你去找的 AI 秘书已经存在，且正在运行。**

接下来的问题，不是「能不能做到」，而是「能走多远」。

---

### 🛠 近期改动计划（2026 Q1）

基于对当前系统的全局分析，从竞品定位、技术架构、用户体验三个维度归纳出四阶段优化方案，按投入产出比排序。

#### Phase A — 稳定性加固（1-2 周）

| #   | 改动                                        | 预期效果                                                |
| --- | ------------------------------------------- | ------------------------------------------------------- |
| A1  | **Hub context 文件 I/O 缓存**（5 分钟 TTL） | 每次 LLM 请求节省 50-100ms，降低磁盘读压力              |
| A2  | **reasoning_queue.jsonl 自动归档**          | 长期运行稳定，防止文件无界增长                          |
| A3  | **deleteFromAssistantHub 真实实现**         | 能删除错误捕捉的卡片，完善闭环                          |
| A4  | **Capture dedup 逻辑接通**                  | `dedupeHint="append_existing"` 追加已有卡片而非重复建卡 |

#### Phase B — 用户体验提升（2-4 周）

| #   | 改动                           | 预期效果                                                      |
| --- | ------------------------------ | ------------------------------------------------------------- |
| B1  | **Token 用量追踪**             | 写 `~/.openclaw/usage.jsonl`，Reviewer 每日统计推送，超额预警 |
| B2  | **Hub context 自适应注入**     | 根据对话意图动态选择注入哪些段落，减少无关 token              |
| B3  | **控制台 Web UI 增强**         | Agent 状态卡片、最近 Capture 列表、Cron Jobs 管理页           |
| B4  | **`openclaw doctor` 健康检查** | 一键验证所有 channel token、LLM 连通性、磁盘空间              |

#### Phase C — 知识图谱升级（1-2 月）

| #   | 改动                    | 预期效果                                                                      |
| --- | ----------------------- | ----------------------------------------------------------------------------- |
| C1  | **知识关联推断**        | Capture 时自动扫描 `people/` `beliefs/`，生成 `links[]`，质变式提升知识连通性 |
| C2  | **Capture 类型可扩展**  | 在 `openclaw.json` 定义自定义 type，无需改源码                                |
| C3  | **间隔重复复习（SRS）** | `belief`/`question` 卡片加入 SM-2 算法，Reviewer 定期触发复习                 |
| C4  | **MCP 接口暴露**        | `tasks`/`people`/`calendar` 成为 MCP resources，接入 Claude Desktop           |

#### Phase D — 开源生态（2-3 月）

| #   | 改动                           | 预期效果                                                 |
| --- | ------------------------------ | -------------------------------------------------------- |
| D1  | **插件沙箱（Worker threads）** | 第三方插件隔离，bug 插件不影响 gateway 主进程            |
| D2  | **CI/CD 激活**                 | vitest 自动运行，capture-agent 覆盖率 4% → 40%           |
| D3  | **Notion / Obsidian 双向同步** | 解锁知识工作者最大的集成缺口                             |
| D4  | **Web clipper**                | 浏览器插件：选中文字 → Kairo capture，消除最后一个摩擦点 |
...
**问题**：待办 App 靠你去打开。日历靠你去翻。邮件靠你去刷新。所有工具都在等你。如果你忘了打开，什么都不会发生。

**跨越时刻**：第一次在没有主动去查的情况下，你在正确的时间收到了正确的信息。

**已完成**：

- Heartbeat 主动推送（晨报 07:10 / 截止提醒 / 逾期扫描 20:30）
- Capture Agent：10 种意图自动分类建卡，置信度 ≥ 85% 静默处理
- 完整任务闭环：说一件事 → 建卡 → 排程 → 提醒 → 自动更新状态，全自动
- Hub Context：每次对话注入 9 个信息源（日历 / 邮件 / 任务 / 人脉 / 决策智慧）
- Gmail + 飞书日历自动接入，每日 07:00 静默同步
- **语意记忆搜索**（`memory_search` / `memory_get`）：LLM 自主从历史记忆中召回相关片段，DashScope text-embedding-v3 向量索引（2026-02-28）
- **Qwen-TTS 语音合成**：每日课程推送 3 条语音气泡（日语朗读 + 英语单词 + 日语单词），qwen3-tts-flash + Cherry 声线，sox 转 OGG 直接发送为 Telegram voice bubble（2026-03-01）
- **3-Agent 架构**（Planner / Executor / Reviewer）：纯配置实现三角分工——Planner 负责与 Ken 对话，Executor 执行所有 cron 任务（课程 / capture / 晨报），Reviewer 负责心跳巡逻与记忆压缩；channel binding 确保消息正确路由（2026-03-01）
- **真正 Multi-Agent `sessions_spawn`**：Planner 可将复杂任务委派给 Executor 子代理（shell 命令、多步骤研究、数据整理）；结果异步自动回报至 Planner；端到端验证完成：Planner 委派 → Executor 在隔离 session 执行工具 → 结果回传 → Planner 汇总给 Ken（2026-03-01）

---

#### 🔄 Phase 1 — 信息主动找到你

**问题**：知识工作者花大量时间在「管理信息流」——主动去 arXiv 搜论文、整理实验结果、跟踪领域动态。这些动作没有创造任何思考价值，却消耗大量注意力。

**跨越时刻**：

> 早上 7:15，今日相关领域新论文出现在你的 Telegram，已翻译、已标注与你研究的关联。你不需要搜索任何东西。
>
> 你说「帮我深度调研 Video World Models 这个方向」，3 分钟后完整报告写入知识库。
>
> 你跑完实验，Kairo 自动比对所有历史结果，告诉你「Exp-C 比 Exp-A 高 2.3%，可能原因是...」

**核心能力**：

| 能力                                                        | 完成后的世界                                                |
| ----------------------------------------------------------- | ----------------------------------------------------------- |
| arXiv 每日监控 + LLM 精选推送                               | 领域新进展主动出现在你面前，你只需判断值不值得深读          |
| `research_deep` LLM tool（原生 TypeScript，不依赖外部服务） | 一句话触发深度调研，报告写入 `04_knowledge/research_notes/` |
| 实验追踪工具（`experiment_log` / `experiment_compare`）     | 每次实验自动记录，异常自动标注，趋势主动汇报                |
| Docker 一键部署                                             | `docker compose up`，降低初始门槛                           |

---

#### Phase 2 — 你的上下文无处不在

**问题**：你用了 5 个 AI 工具，每一个都不认识你。你的截止日在 Kairo，但 Claude Desktop 不知道；你的研究方向在这里，但 GPT-4 不知道。**你是 5 个陌生人的雇主，每次对话都要重新介绍自己。**
...
| 能力             | 完成后的世界                                                    |
| ---------------- | --------------------------------------------------------------- |
| Kairo MCP Server | 你的 calendar / tasks / people 成为任何 AI 工具可读取的标准接口 |
| MCP Client       | Kairo 调用社区 MCP server（GitHub / 网络搜索），不重造轮子      |
| Self-Improving   | 每周分析自身判断错误 → 自动更新行为模式，系统随时间越来越准     |

---

#### Phase 3 — 整件事可以委派

**问题**：现在最好的结果是「AI 帮你做一步」。帮你调研，你还要整合；帮你写草稿，你还要喂内容；帮你分析，你还要下结论。每次都是线性的，不是乘法。

**跨越时刻**：

> 你说：「帮我准备这周组会的 slides，主题是 Video Understanding 最新进展。」
>
> 三小时后，一份可以直接使用的 slides 草稿推到你的 Telegram。你什么都没做，只是说了一句话。

**核心能力**：

| 能力                                             | 完成后的世界                                                                   |
| ------------------------------------------------ | ------------------------------------------------------------------------------ |
| Swarm 调度层（OpenAI Agents SDK / agency-swarm） | Kairo 作为 orchestrator，把任务拆解分派给 Research / Code / Writing 专职 agent |
| Bounded Autonomy                                 | 高风险操作（发邮件 / 删文件 / 提交代码）必须你确认；其他自主完成               |
| Audit Trail                                      | 所有 agent 行动写入 `audit.jsonl`，任何时刻可回溯                              |
...
> 一个不会写代码的人运行 `./install.sh`，在 5 分钟内拥有一个运行在自己机器上、数据完全私有、零订阅费的个人 AI 秘书。

**已完成（2026-03-02）**：

- ✅ 去除 root 依赖，`$KAIRO_HOME` 全链路可配置（`src/config/paths.ts` + `hub.ts` + `exec-approvals.ts`）
- ✅ `install.sh` repo root 入口：Node≥22 检查 → pnpm install → UI build → gateway build → 交互式 setup wizard
- ✅ `openclaw.example.json` 最小配置模板，clone 后即可参考
- ✅ 3-Agent 架构（Planner/Executor/Reviewer）纯配置分工，sessions_spawn 端对端验证完成
...
向 @YourBot 发一条消息，试试说「提醒我明天早上回复 Jason」。
```
...
- **Self-Improving**（从 Phase 2 延续深化）：每周分析判断错误 → 更新规则 → 下次更准
- **Computer Use**：不只给文字建议，直接操作浏览器完成任务（填表报名、下载报告、网页操作）
- **关系图谱**：人脉卡片升级为网络图，自动发现「Jason 和 Tom 都在做 AI 教育，可能值得互相介绍」
- **长期记忆**：月度压缩 → 年度精华，你积累的每一个想法永不随记忆消退
- **跨设备 Context**：phone / laptop / server，同一个 Kairo 上下文
