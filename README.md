...
> **"所有 AI 工具都是你使用的东西。Kairo 的方向是成为你依赖的基础设施。"**

区别在哪里？**电话是工具**——你拨出去才有人接，你挂掉连接断开。**电网是基础设施**——不论你在不在，它都在运作。

现有的所有 AI——ChatGPT、Claude、Notion AI、各种 Agent 框架——本质上都是电话：再聪明，也只有你主动拨打才有价值。Kairo 要做的是电网：一套持续运行在你服务器上的个人 AI 协调层。它在你睡觉时处理邮件摘要，在你开会时追踪到期任务，在你分心时提醒你三天前遗忘的那件事。

---

### 系统设计哲学

六个能力层，每层有对应的生物类比，每层有可观测的输入与输出：

| 能力层       | 类比       | Kairo 组件                                                                       | 状态 |
| ------------ | ---------- | -------------------------------------------------------------------------------- | :--: |
| **捕获**     | 感官器官   | Capture Agent — 10 种意图；置信度 ≥ 85% 静默建卡，低于阈值显示确认菜单           |  ✅  |
| **记忆**     | 海马体编码 | Hub Context — 9 个信息源，5600 字上限，注入每次对话                              |  ✅  |
| **主动调度** | 前额叶规划 | Heartbeat Engine + CronService — 事件触发 + 定时排程，晨报 / 截止提醒 / 逾期扫描 |  ✅  |
| **推理执行** | 皮层推理   | LLM agent turn — 全 Hub Context 携带，5 分钟超时保护，原子写入防崩溃             |  ✅  |
| **自我监控** | 免疫系统   | Watchdog — 静默失败检测：cron 未运行 / token 过期 / Heartbeat 超时 → 推送告警    |  🔄  |
| **自我进化** | 突触可塑性 | Accuracy Log — 意图分类 ✅/❌ 月度追踪，准确率曲线写入 Git，不可篡改             |  ⬜  |

---

### 已验证的核心假设

Kairo v0.9 正在 Ken 的服务器上生产运行，三件关键事已在真实环境中验证：

1. **主动推送可行** — 每天 07:10 晨报准时推到 Telegram，Ken 不需要做任何事
2. **完整闭环可行** — 说一件事 → 建卡 → 排程 → 到时间主动提醒 → LLM 自动更新状态，全程无需人工干预
3. **持久上下文可行** — 每次对话注入 9 个信息源（5600 字），Kairo 已感知今日日历、未完成任务、相关人脉，不需要你重复介绍自己

这三件事合在一起意味着：**一个不需要你去找的 AI 秘书已经存在，且正在运行。**

---

### 变革路径

每一个 Phase 不是「增加功能」，而是一个**之前不可能、之后变成理所当然**的跨越：

```
Phase 0 ✅  你不需要记得去查     → AI 第一次主动找到你
Phase 1 🔄  信息主动找到你       → 你不再需要管理信息流
Phase 2     你的上下文无处不在   → 任何 AI 工具都认识你
Phase 3     整件事可以委派       → 你从执行者变成审批者
Phase 4     任何人都能拥有       → 私有 AI 秘书不再是技术特权
Phase 5     它开始改进自己       → 越来越了解你的伙伴，而不是工具
```
...
**问题**：所有工具都在等你打开。如果你忘了打开，什么都不会发生。

**跨越时刻**：第一次在没有主动去查的情况下，你在正确的时间收到了正确的信息。

**已完成**：

- **Capture Agent** — 自然语言 → 10 种意图分类；置信度 ≥ 85% 静默建卡 + 写入 `tasks/`；低于阈值显示确认菜单；`type=timeline/action` + 未来 due date → 自动创建 cron job，以 `capture id:{id}` 去重
- **Heartbeat Engine** — 07:10 晨报 / 截止提醒 / 20:30 逾期扫描；5 分钟 LLM 超时保护；`HEARTBEAT_OK` sentinel 避免无意义空推送
- **Hub Context** — 9 段上下文注入每次对话：今日日历 / 邮件摘要 / 未完成任务（8 行）/ 追踪中 / 路线图 / 月摘要 / 人脉卡片（3 个）/ 行为模式 / 决策智慧（7 位）；总上限 5600 字
- **飞书日历 LLM tool** — `create_event / list_events`；mutex 防并发写入；user token 30 天自动滚动刷新；写入失败原子回滚
- **多邮箱接入** — Gmail（Pub/Sub webhook）+ 飞书日历（OAuth）+ Outlook/163/QQ/iCloud（IMAP 自动识别域名）；`hub-context.ts` 动态扫描所有 `*-mail.md`，新增邮箱无需改代码

**当前缺口（🔴 阻塞公开部署）**：

- **`$KAIRO_HOME` 可配置** — 路径现在 hardcoded `/opt/LazyingArtBot/`，外部用户无法部署
- **Watchdog 告警** — 静默失败无通知；cron 未运行 / Heartbeat 超时 / token 过期均需主动检测 + 推送告警

---

#### 🔄 Phase 1 — 信息主动找到你

**问题**：知识工作者花大量时间管理信息流——主动去 arXiv 搜论文、整理实验结果、跟踪领域动态。这些动作消耗大量注意力，却没有创造任何思考价值。

**跨越时刻**：

> 早上 7:15，今日相关领域新论文出现在你的 Telegram，已翻译、已标注与你研究的关联。你不需要搜索任何东西。
>
> 截止日还有 47 小时，Kairo 从日常模式切换为紧急模式，每 4 小时汇报进度缺口。

**核心能力**：

- **arXiv 每日监控** — 读取 `research_topics.md` 关键词，拉取过去 24 小时提交论文，LLM 精选 ≤ 5 篇并标注与当前研究的相关度；推送 + 写入 `arxiv-digest.md`；07:15 cron
- **`research_deep` LLM tool** — 触发：用户一句话描述调研方向；执行：web search + arXiv API + PDF 解析 → 结构化报告写入 `research_notes/{topic}-{date}.md`；hub-context 自动注入最新 3 篇；不依赖外部服务，原生 TypeScript 实现
- **`experiment_log` / `experiment_compare` LLM tools** — `log`：记录实验参数 + 指标 → `experiments/{id}.md`；`compare`：自动生成对比表格，标注当前最优；晨报中主动 diff 前日结果
- **截止日紧急模式** — 触发：距任意 deadline ≤ 48 小时；行为：Heartbeat 频率从每日切换为每 4 小时，晨报改为「距 deadline X 小时，还差什么」；deadline 过后自动回归正常节奏
- **Docker 一键部署** — `docker compose up`；挂载 `~/.kairo:/data`；`restart: unless-stopped`；降低初始部署门槛

---

#### Phase 2 — 你的上下文无处不在

**问题**：你用了 5 个 AI 工具，每一个都不认识你。你的截止日在 Kairo，但 Claude Desktop 不知道；你的人脉卡片在这里，但 GPT-4 不知道。**你是 5 个陌生人的雇主，每次对话都要重新介绍自己。**

**跨越时刻**：

> 打开 Claude Desktop，它已经知道你今天的任务、截止日、正在追踪的事项。不是你告诉它的——是 Kairo 在后台，通过 MCP 协议，让你的上下文无处不在。

**核心能力**：

- **Kairo MCP Server** — 暴露 Tools：`get_tasks(status?)`、`create_task(title, due, type)`、`get_calendar(days=7)`、`get_waiting()`、`get_person(name)`；暴露 Resources：`roadmap://current`、`daily_log://today`；任何 MCP client（Claude Desktop / Cursor / ChatGPT）直接接入，无需额外授权
- **MCP Client** — Kairo 作为 client 接入外部 MCP server：`server-filesystem`（替换现有 file tools）、`server-github`（PR / issue / commit 读取）、Brave Search MCP（实时搜索）；不重造已有轮子
- **Accuracy Log + Self-Improving** — 每次意图分类结果写入 `self_review/accuracy.jsonl`；用户 ✅/❌ 确认触发标注；每月统计准确率 + 最常误判类型 + 改进建议；结果写入 `self_review/YYYY-MM.md`，**永久存在 Git，不可篡改**；准确率曲线随时间可见，用数字证明系统在进步

---

#### Phase 3 — 整件事可以委派

**问题**：现在最好的结果是「AI 帮你做一步」。每次都是线性叠加，不是乘法。

**跨越时刻**：

> 「帮我准备这周组会的文献综述，主题是 Video Understanding 最新进展。」
>
> 三小时后，一份结构完整的综述草稿推到你的 Telegram。**你的工作是最后 15 分钟的确认与修改，不是前面 3 小时的调研与整理。**
>
> _注：Phase 3 的输出质量由 Phase 1（research tools）和 Phase 2（个人上下文）的完成度决定。_

**核心能力**：

- **Swarm 调度层** — Kairo 作为 orchestrator 解析意图 + 拆解子任务；专职 worker：Research（arXiv + `research_deep`）/ Code（实验脚本）/ Writing（草稿生成）/ Email（起草发送）；基于 OpenAI Agents SDK；每个 worker 有独立 system prompt + tool scope + token budget
- **Bounded Autonomy** — 高风险操作白名单（对外发送邮件 / 删除文件 / 提交代码 / 修改 cron config）→ 必须 Ken 确认后执行；其余操作自主完成；确认请求推送到 Telegram，30 分钟无响应自动挂起
- **Audit Trail** — 所有 agent 行动写入 `audit.jsonl`：`{ts, agent, action, input_summary, output_summary, status}`；Control UI 可视化展示；支持任意时刻回溯
- **优雅降级** — 任意 worker 失败 → orchestrator 记录原因 + 推送告警，不静默放弃；支持单 worker 重试而不重跑整个任务

---

#### Phase 4 — 任何人都可以拥有

**问题**：数据目录 hardcoded，依赖 root 权限。**私有 AI 秘书变成了技术人员的专属特权。**

**跨越时刻**：

> 一个不会写代码的人运行 `./install.sh`，5 分钟内拥有一个运行在自己机器上、数据完全私有、零订阅费的个人 AI 秘书。

**核心能力**：

- **`$KAIRO_HOME` 可配置** — 所有路径替换为 `process.env.KAIRO_HOME ?? path.join(os.homedir(), '.kairo')`；systemd service 改为普通用户运行；`~/.openclaw/` → `$KAIRO_HOME/data/` 迁移脚本
- **`install.sh`** — 交互式 5 步 setup：检查依赖（Node 22+）→ 选择频道 → 输入 token → 选择模型 → 生成配置 + 启动服务；可选步骤：Gmail OAuth / 飞书日历 / 飞书频道
- **GitHub Release v1.0** — `openclaw.example.json` 完整模板 + QUICKSTART.md 7 步 + Docker image 发布

---

#### Phase 5 — 它开始改进自己（长期愿景）

> **到达这里，Kairo 不是「我使用的工具」，而是「一个随时间越来越了解我的伙伴」。**

- **Self-Improving 深化** — 从 Accuracy Log（Phase 2）延伸：每次改进有明确 before/after 准确率对比，改动写入 `IDENTITY.md` / `HEARTBEAT.md` playbook，用数字证明它在进步
- **Computer Use** — 接入 Anthropic Computer Use API 或 browser-use；隔离 Chromium 实例执行；截图日志保留；高风险操作仍需确认
- **关系图谱** — `people/` 卡片升级为有向图；自动推断潜在连接（共同话题 / 共同项目）；Obsidian Graph View 兼容格式
- **长期记忆** — 月度压缩 → 年度精华；向量索引支持自然语言查询 60 天知识窗口
- **跨设备 Context** — `/api/context` endpoint 返回当前 Hub Context 快照；与 MCP Server（Phase 2）叠加，任何设备接入同一个 Kairo 上下文
...
| 项目                                                                                    | 用途                     | Phase |
| --------------------------------------------------------------------------------------- | ------------------------ | :---: |
| [SakanaAI/AI-Scientist-v2](https://github.com/SakanaAI/AI-Scientist-v2)                 | 自主论文生成流程参考     |   1   |
| [SamuelSchmidgall/AgentLaboratory](https://github.com/SamuelSchmidgall/AgentLaboratory) | 文献 → 实验 → 写作三段式 |   1   |
| [openai/openai-agents-python](https://github.com/openai/openai-agents-python)           | Swarm 官方后继框架       |   3   |
| [VRSEN/agency-swarm](https://github.com/VRSEN/agency-swarm)                             | multi-agent 执行层备选   |   3   |
| [browser-use/browser-use](https://github.com/browser-use/browser-use)                   | 浏览器操作能力           |   5   |
