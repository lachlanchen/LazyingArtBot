...
function parseWhatsappGroupId(ctx: FinalizedMsgContext): string | undefined {
  if (ctx.ChatType !== "group") {
    return undefined;
  }
  const from = (ctx.From ?? ctx.OriginatingTo ?? "").trim();
  if (from) {
    return from;
  }
...
export function toWhatsAppCaptureInput(ctx: FinalizedMsgContext): CaptureInput {
  const content =
  ...
  return {
    ...
    metadata: {
      platform: "whatsapp",
      messageId,
      groupId: parseWhatsappGroupId(ctx),
