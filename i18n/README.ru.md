...
>
> Примечание: каталог `i18n/` существует и сейчас включает Arabic. Дополнительные локализованные варианты README добавляются по одному, чтобы сохранять согласованность с обновлениями исходника.

**LazyingArtBot** — это мой персональный стек AI-ассистента для **lazying.art**.  
Он построен на базе OpenClaw и адаптирован под мои ежедневные сценарии: многоканальный чат, local-first управление и автоматизация email → calendar/reminder/notes.
...
| Область | Текущая базовая конфигурация в этом репозитории |
| --- | --- |
...
- Web dashboard / web chat как поверхность управления.
...
Опциональная глобальная установка CLI (соответствует quick-start):
...
3. Задайте как минимум один ключ провайдера модели (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY` и т.д.).
...
- Операции чтения/сохранения:
  создание и обновление выходов Notes, Reminders и Calendar для операций AutoLife.
...
- Ещё раз сверяйте значения `.env` с `.env.example`.
